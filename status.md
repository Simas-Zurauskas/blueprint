...
4. **Read the run log** — the source of the last run's date, the only source for when a question was
   ...
   **Fallback, only when there is no readable log:** say plainly that flag ages could not be computed and
   that **locked-ness could not be determined** — never present the Blueprint as unlocked because the log
   could not be read.
...
| **C6** | **Nobody has confirmed these** | Counts only, and the headline is the sum of the breakdown, not a subset of it: every row still at `Confirmed = AI generated` — features and questions — plus every feature at `Intent = Agreed` with an empty `Confirmed by`, plus every question at `Answered` with an empty `Confirmed by`, **plus every feature named by a `STALE AGREEMENT` line in the run log** ([`add.md`](add.md) A5) — a run wrote into it while it was `Agreed`. Read that line; **never try to derive when a feature was agreed**, which nothing records and no run may claim to know ([`spec/notion-mechanics.md`](spec/notion-mechanics.md) §5). The line is written once and stops being counted when a human re-agrees the feature and says so. That last one is its own line, not a number in the total: the feature still reads `Agreed` over a sentence nobody agreed, no run may re-agree it, and until a person does the label is claiming something untrue ([`add.md`](add.md) A2 item 4). Print the breakdown beside the total, so the one number measuring unread machine output cannot read reassuring by leaving categories out |
...
4 WILL NOT APPLY NEXT TIME (C3) · 5 BLOCKING LINKS (C5) · 6 PROPOSALS WAITING ON YOU (C2) · 7 STUCK AND
GOING STALE (C7) · 8 THE FRONT DOOR (C8) · 9 NOBODY HAS CONFIRMED THESE (C6). C9 prints first because it
is the only line about something that should not be in the document at all; every other line is about
something that is wrong. It is also, on almost every run, absent.

...
Both are gone. A feature row is the spec, and the document is settled as a whole by the lock — there is
no per-feature state to keep in step with anything.

**The lock is the sign-off, and it is the only one** (v13). There is no per-feature agreement axis:
a human settles the whole document at once ([`../lock.md`](../lock.md)), acknowledging every open
marker and unratified default item by item on the way. **Nothing in this system blocks anything** —
`lock.md` is explicit that locking over an unsettled item is allowed, only never silently.
...
finds 44.4% of real gaps"* is a **recall** figure. It was previously used to conclude that "most of what
...
**A feature is lock-ready when:** it carries no open marker in its body,
...
- **A per-feature agreement axis, and a question-approval state — both removed at v13.** `Intent`
  (`Draft`/`Agreed`) and the `Proposed` status are gone. The lock is the only sign-off: a human settles
  the whole document at once rather than feature by feature, and a generated question is live from the
  moment it passes the admission gate. **A marker therefore blocks nothing** — it is an admitted gap,
  counted by [`../status.md`](../status.md) C5 and acknowledged item by item at
  [`../lock.md`](../lock.md) L2, which is what `lock.md` always said locking was: allowed over an
  unsettled item, never silent about one.
...
- **A `Confirmed` select (`AI generated` · `Human approved`), recording drafting provenance and gating
  ...
  What survives the removal: a run still never locks a Blueprint, never sends a client packet it

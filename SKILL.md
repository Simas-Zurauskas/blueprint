...
**Nothing else is on this list, and v17 through v33 and v35 are deliberately not** — each changed rules,
...
6. **An independent check is a separate dispatch, not a separate paragraph.** Wherever a phase requires a
   ...
   **(i)** an agent-spawn tool, where the harness exposes one; **(ii) the shell** — a harness that can
   ...
   one fresh reader per drafted question, given the whole document and nothing about the routing, whose

...
**Nothing else is on this list, and v17 through v33, v35 and v36 are deliberately not** — each changed rules,
...
the single home of where it lives: `<home>`, normally **`wiki-{project}/blueprint/`** in the project's
docs repo — the same place, for the same reason, that eng-rulebook keeps a project's profiles and
audits — and `.blueprint/` in the workspace only where the project has no wiki folder (v33; until then
...
6. **An independent check is a separate dispatch, not a separate paragraph.** Wherever a phase requires a
   ...
   **(i)** an agent-spawn tool, where the harness exposes one; **(ii) the shell** — a harness that can
   ...
   open to asking, never to silence. **A second pre-write dispatch follows it — Q4's cold read** (v32):
   fresh readers, up to ten drafted rows each, given the document and nothing about the routing, whose
   five verdicts and the evidence each must carry are Q4's to define and are not restated here. It is a
   separate dispatch under this rule like any other: with none available the row is written
   `unverified`, never read back by the context that drafted it and called a read.
...
8. **Parallel compute, serial commit.** Sub-agent dispatches whose briefs are read-only may run
   ...
   **(i) the orchestrating run performs every read and write of the target itself.** A sub-agent is
   briefed with the data it needs and never reads or writes the target or the working folder's
   record; **the one file it may read is a brief the orchestrator froze for it** (v36) — written to the
...
| Run | Grilling ([`questions.md`](questions.md) Q2) | Verification ([`questions.md`](questions.md) Q4) |
|---|---|---|
| an `add` handoff, `init`'s closing handoff (I7), and a bare `/blueprint questions` | the delta scale — **at most four dispatches** | one disposition-and-directions dispatch per 25 candidates · one cold reader per 10 drafted rows |
| `init`'s skeleton grill (I2), and `/blueprint questions full` | the full scale — **one dispatch per `Area`, plus two** | the same |
...
a body's hash comes from the read-back already in hand.

**A pause is ended only by a human.** A run schedules nothing to wake itself — no timer, no loop, no
reminder to continue — and a paused entry resumes when a person says so. (*Never runs on a schedule*,
below, is the same rule seen from the other side.)
